...
<div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-col md:flex-row gap-6">
            ...
            <div class="w-full md:w-2/3">
                                ...
                                <tr class="border-b">
                                    <td class="py-4">
                                        <div class="flex items-center">
                                            <img src="{{ $item['image'] }}" alt="{{ $item['name'] }}" class="h-16 w-16 object-cover rounded mr-4">
                                            <div>
                                    ...
                                    <td class="text-center py-4">Rp {{ number_format($item['price'] * $item['quantity'], 0, ',', '.') }}</td>
                ...
                <div class="flex justify-between mt-4">
                    <a href="{{ route('home') . (request()->getQueryString() ? '?' . request()->getQueryString() : '') }}" class="bg-gray-200 text-gray-800 py-2 px-4 rounded hover:bg-gray-300">
                        Continue Shopping
                    </a>
                    <form action="{{ route('cart.clear', ['mejaId' => $table->id ?? '']) }}" method="POST">
                        @csrf
            ...
            <div class="w-full md:w-1/3">
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="mt-6">
                        <a href="{{ route('payment.index') . (request()->getQueryString() ? '?' . request()->getQueryString() : '') }}" class="block w-full bg-teal-600 text-white text-center py-3 px-4 rounded-md hover:bg-teal-700">
                            Proceed to Payment
                        </a>
                    </div>
                </div>
    ...
    @else
        <div class="bg-white rounded-lg shadow-md p-6 text-center">
            ...
            <a href="{{ route('home') . (request()->getQueryString() ? '?' . request()->getQueryString() : '') }}" class="bg-teal-600 text-white py-2 px-4 rounded-md hover:bg-teal-700">
                Start Shopping
            </a>
        </div>
    @endif
